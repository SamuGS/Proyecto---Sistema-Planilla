...
class Empleado {
	...
	public static function mostrar_pago(){
		...
		while($r = $query[0]->fetch_array()){
			...
			$array[$cnt]->id = $r['id'];
			$array[$cnt]->nombre = $r['nombre'];
			$array[$cnt]->mensual = $r['mensual'];
			$array[$cnt]->quincenal = $r['quincenal'];
			$array[$cnt]->afp = $r['afp'];
			$array[$cnt]->seguro = $r['seguro'];
	...
	public static function mostrar_usuario_inactivo(){
		$sql = "select * from usuario where estado = 2";
		$query = Executor::doit($sql);
		$array = array();
		$cnt = 0;
		while($r = $query[0]->fetch_array()){
	...
	public static function ctn(){
		$cnt = mysql_num_rows(mysql_query('select * from asistencias where estado = 1'));
